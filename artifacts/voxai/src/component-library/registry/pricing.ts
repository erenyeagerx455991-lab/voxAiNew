...
export const pricingComponents: ComponentDef[] = [
  {
    ...
    tags: ['3-tier', 'toggle', 'popular-badge', 'dark', 'glassmorphism'],
    description: 'Dark 3-tier pricing with monthly/yearly toggle, popular card highlighted',
    priority: 10,
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-12">
          ...
          <div className="inline-flex items-center gap-3 bg-white/5 border border-white/10 rounded-full p-1">
            ...
            <button onClick={() => setYearly(true)} className={\`px-4 py-2 rounded-full text-sm font-medium transition-all \${yearly ? 'bg-white text-black' : 'text-gray-400'}\`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
          {plans.map((plan, i) => (
            <div key={plan.name} className={\`relative rounded-2xl p-6 border transition-all duration-300 \${plan.popular ? 'bg-gradient-to-b from-violet-900/60 to-blue-900/40 border-violet-500 scale-105 shadow-2xl shadow-violet-500/20' : 'bg-white/5 border-white/10 hover:border-white/20'}\`}>
              {plan.popular && (
              ...
              <button className={\`w-full py-3 rounded-xl font-semibold text-sm transition-all mb-6 \${plan.popular ? 'bg-gradient-to-r from-violet-600 to-blue-600 text-white hover:opacity-90' : 'border border-white/20 text-white hover:bg-white/10'}\`}>
                {plan.cta}
              </button>
  ...
  },
  {
    ...
    tags: ['2-tier', 'light', 'minimal', 'clean'],
    description: 'Light minimal 2-tier pricing with comparison table',
    priority: 7,
    ...
    <section className="py-24 bg-gray-50">
      <div className="max-w-4xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {plans.map(plan => (
            <div key={plan.name} className={\`rounded-2xl p-8 border \${plan.highlight ? 'bg-gray-900 border-gray-900 shadow-xl shadow-gray-900/20' : 'bg-white border-gray-200'}\`}>
              <h3 className={\`font-bold text-2xl mb-1 \${plan.highlight ? 'text-white' : 'text-gray-900'}\`}>{plan.name}</h3>
              ...
              <button className={\`w-full py-3.5 rounded-xl font-semibold text-sm transition-all mb-8 \${plan.highlight ? 'bg-violet-600 hover:bg-violet-500 text-white' : 'border-2 border-gray-900 text-gray-900 hover:bg-gray-900 hover:text-white'}\`}>
                {plan.cta}
              </button>
