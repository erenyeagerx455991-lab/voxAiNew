...
export const SECTION_TEMPLATES: any[] = [
      ...
      <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-16 items-start">
        ...
        <div className="grid grid-cols-2 gap-4 pt-20">
          ...
          <div className="col-span-2 bg-gradient-to-r from-[#635BFF]/15 to-[#00D4FF]/10 border border-[#635BFF]/30 rounded-2xl p-5 flex items-center justify-between">
            ...
            <button className="bg-gradient-to-r from-[#635BFF] to-[#00D4FF] text-white text-xs font-bold px-5 py-2.5 rounded-full hover:opacity-90 transition-all shrink-0">CTA_PRIMARY →</button>
          </div>
        </div>
      </div>
    </section>
    ...
    <section className="py-24 bg-[#0B0B0B]">
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          ...
          <p className="text-white/35 text-base max-w-xs md:text-right">FEATURES_SUBHEADING</p>
        </div>
        <div className="grid grid-cols-12 gap-3">
          <div className="col-span-12 md:col-span-8 bg-gradient-to-br from-[#FF3D57] to-[#FF6B35] rounded-3xl p-10 relative overflow-hidden min-h-[280px] flex flex-col justify-between hover:scale-[1.01] transition-transform">
            ...
            <p className="text-white/70 text-sm leading-relaxed max-w-md">{cards[0].desc}</p>
          </div>
          <div className="col-span-12 md:col-span-4 bg-[#141414] border border-white/10 rounded-3xl p-8 flex flex-col justify-between min-h-[280px] hover:border-white/20 transition-colors">
            <span className="bg-white/8 text-white/50 text-xs font-bold px-3 py-1.5 rounded-full uppercase tracking-wider w-fit">{cards[1].tag}</span>
            <div>
              <h3 className="text-white text-2xl font-black mb-3">{cards[1].title}</h3>
              <p className="text-white/35 text-sm leading-relaxed">{cards[1].desc}</p>
            </div>
          </div>
          {cards.slice(2).map((c, i) => (
            <div key={i} className="col-span-12 md:col-span-6 bg-[#141414] border border-white/10 rounded-3xl p-8 hover:border-white/20 transition-colors">
              <span className="bg-white/8 text-white/50 text-xs font-bold px-3 py-1.5 rounded-full uppercase tracking-wider">{c.tag}</span>
    ...
    <section className="py-24 bg-[#0F0F0F]">
      <div className="max-w-5xl mx-auto px-6">
        <div className="flex items-end justify-between mb-16 border-b border-white/8 pb-10">
          ...
          <p className="text-white/30 text-sm max-w-xs text-right hidden md:block">FEATURES_SUBHEADING</p>
        </div>
        {feats.map((f, i) => (
          <div key={i} className="group grid grid-cols-12 gap-6 border-b border-white/8 py-8 hover:border-[#5E6AD2]/40 transition-colors cursor-default">
            <div className="col-span-2 md:col-span-1">
              <span className="text-white/10 font-black text-5xl leading-none select-none group-hover:text-[#5E6AD2]/30 transition-colors">{f.n}</span>
            </div>
            <div className="col-span-10 md:col-span-5 flex flex-col justify-center">
              <h3 className="text-white font-bold text-xl mb-1 group-hover:text-[#5E6AD2] transition-colors">{f.title}</h3>
              <span className="text-[#5E6AD2] text-xs font-semibold tracking-wider uppercase">{f.tag}</span>
            </div>
      ...
      visual: (
        <div className="bg-[#111] border border-white/10 rounded-xl overflow-hidden">
          ...
          <div className="p-5 font-mono text-xs space-y-2">
            ...
            <div className="text-white/40">✓ COMMAND_OUTPUT_1</div>
            <div className="text-white/40">✓ COMMAND_OUTPUT_2</div>
    ...
    },
    {
      ...
      visual: (
        <div className="bg-[#111] border border-white/10 rounded-xl p-5">
          <div className="text-white/25 text-xs mb-3 font-semibold uppercase tracking-wider">PREVIEW_LABEL</div>
          <div className="space-y-2">
            {['STATUS_1','STATUS_2','STATUS_3','STATUS_4'].map((s,i) => (
              <div key={i} className="flex items-center gap-3 p-2.5 rounded-lg bg-white/5">
                ...
                <span className="ml-auto text-white/25 text-xs font-mono">{'just now'}</span>
              </div>
            ))}
          </div>
        </div>
    ...
    <section className="bg-black">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center py-20 border-b border-white/8">
          ...
          <p className="text-white/35 text-base max-w-lg mx-auto">FEATURES_SUBHEADING</p>
        </div>
        {rows.map((row, i) => (
          <div key={i} className={\`grid md:grid-cols-2 gap-0 border-b border-white/8 \${i % 2 === 1 ? 'md:flex-row-reverse' : ''}\`}>
            <div className={\`p-12 flex flex-col justify-center \${i % 2 === 1 ? 'md:order-2' : ''}\`}>
              <p className="text-white/30 text-xs font-semibold tracking-widest uppercase mb-3">{row.eyebrow}</p>
              <h3 className="text-white text-2xl md:text-3xl font-black mb-4 tracking-tight">{row.title}</h3>
              <p className="text-white/35 text-sm leading-relaxed">{row.desc}</p>
            </div>
...

  {
    ...
    standaloneCode: `function Features() {
  const steps = [
    { n: '1', title: 'STEP_1_TITLE', desc: 'STEP_1_DESC', icon: '◆', color: 'from-violet-500 to-purple-600' },
    { n: '2', title: 'STEP_2_TITLE', desc: 'STEP_2_DESC', icon: '↗', color: 'from-blue-500 to-cyan-500' },
    { n: '3', title: 'STEP_3_TITLE', desc: 'STEP_3_DESC', icon: '◈', color: 'from-emerald-500 to-teal-500' },
    { n: '4', title: 'STEP_4_TITLE', desc: 'STEP_4_DESC', icon: '◉', color: 'from-pink-500 to-rose-500' },
  ];
  return (
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid md:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <button className="mt-10 bg-white text-black font-bold px-7 py-3.5 rounded-xl hover:bg-white/90 transition-all text-sm">CTA_PRIMARY →</button>
          </div>
          <div className="bg-[#111118] border border-white/10 rounded-2xl overflow-hidden shadow-2xl">
            ...
            <div className="p-5">
              ...
              <div className="bg-white/5 border border-white/8 rounded-xl p-4">
                <div className="flex items-center justify-between mb-3"><span className="text-white/50 text-xs">CHART_LABEL</span><span className="text-white/25 text-xs">This month</span></div>
  ...
  const logs = [
    { t: '00:01', c: 'text-white/30', m: 'Cloning repository...' },
    { t: '00:02', c: 'text-white/30', m: 'Installing dependencies' },
    { t: '00:04', c: 'text-white/40', m: 'Running build command' },
    { t: '00:05', c: 'text-[#00FFF0]', m: '$ npm run build' },
    { t: '00:08', c: 'text-white/40', m: 'Compiling 47 modules...' },
    { t: '00:11', c: 'text-white/40', m: 'Optimizing assets' },
    ...
    <section className="py-20 bg-black">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-12">
          ...
          <p className="text-white/35 text-base max-w-xl mx-auto">PREVIEW_SUBHEADING</p>
        </div>
        <div className="border border-white/10 rounded-2xl overflow-hidden bg-[#111] shadow-2xl">
          <div className="flex items-center gap-2 px-5 py-3 bg-[#0a0a0a] border-b border-white/8">
            ...
            <span className="text-white/25 text-xs ml-3">SITE_NAME · Deployments</span>
          </div>
          <div className="flex">
            <div className="w-64 border-r border-white/8 shrink-0">
              <div className="px-4 py-3 border-b border-white/5"><span className="text-white/30 text-xs font-semibold uppercase tracking-wider">Projects</span></div>
              {projects.map((p, i) => (
                <div key={i} className={\`px-4 py-3.5 border-b border-white/5 last:border-0 hover:bg-white/5 cursor-pointer transition-colors \${i === 1 ? 'bg-white/5' : ''}\`}>
                  ...
                  <div className="flex items-center justify-between pl-4">
                    <span className="text-white/25 text-xs font-mono">{p.branch}</span>
                    <span className="text-white/20 text-xs">{p.time}</span>
                  </div>
                </div>
              ))}
            </div>
            <div className="flex-1 bg-[#0a0a0a] p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-white/20 text-xs font-mono">feat/dashboard</span>
              </div>
              <div className="bg-black/50 rounded-xl border border-white/8 p-4 font-mono text-xs space-y-1.5 min-h-[200px]">
                {logs.map((l, i) => (
                  <div key={i} className="flex gap-3">
                    <span className="text-white/15 shrink-0">{l.t}</span>
                    <span className={l.c}>{l.m}</span>
                  </div>
                ))}
                <div className="text-white/15 animate-pulse">_</div>
              </div>
            </div>
          </div>
        </div>
  ...
  const cols = [
    {
      name: 'Todo', count: 4, color: 'text-white/40',
      items: [
        ...
        { title: 'ISSUE_3_TITLE', label: 'ISSUE_3_LABEL', priority: '●', pColor: 'text-white/20', avatar: 'C' },
      ]
    },
    {
    ...
    },
    {
      ...
      items: [
        { title: 'ISSUE_6_TITLE', label: 'ISSUE_6_LABEL', priority: '●', pColor: 'text-white/20', avatar: 'B' },
        { title: 'ISSUE_7_TITLE', label: 'ISSUE_7_LABEL', priority: '●', pColor: 'text-white/20', avatar: 'C' },
      ]
    },
  ];
  return (
    <section className="py-20 bg-[#0F0F0F]">
      <div className="max-w-7xl mx-auto px-6">
        <div className="mb-12">
          ...
          <p className="text-white/30 text-base max-w-xl">PREVIEW_SUBHEADING</p>
        </div>
        <div className="bg-[#111] border border-white/8 rounded-2xl overflow-hidden shadow-2xl">
          <div className="flex items-center gap-4 px-5 py-3 border-b border-white/8 bg-[#0d0d0d]">
            <span className="text-white/50 text-sm font-semibold">TEAM_NAME</span>
            <div className="flex gap-1 ml-auto">
              {['Board','List','Timeline'].map(v => <button key={v} className={\`text-xs px-3 py-1.5 rounded-md \${v==='Board'?'bg-white/10 text-white':'text-white/30 hover:text-white/50'} transition-colors\`}>{v}</button>)}
            </div>
          </div>
            ...
            {cols.map((col, ci) => (
              <div key={ci} className="p-4">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-white/20 text-xs ml-auto">{col.count}</span>
                </div>
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="bg-[#0D1E35] border border-white/10 rounded-2xl overflow-hidden shadow-2xl">
          <div className="flex items-center gap-2 px-5 py-3 bg-[#091726] border-b border-white/8">
            ...
            <span className="text-white/20 text-xs ml-3">app.SITE_NAME.com/dashboard</span>
          </div>
          <div className="p-6 space-y-5">
            ...
            <div className="bg-[#0F3460]/40 border border-white/8 rounded-xl p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-white/25 text-xs">Last 12 months</span>
              </div>
                ...
                <div key={i} className="flex items-center justify-between px-4 py-3 border-b border-white/5 last:border-0 hover:bg-white/5 transition-colors">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-white/25 text-xs">{t.time}</div>
                    </div>
                  </div>
    ...
    <section className="py-20 bg-[#0B0B0B]">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="bg-[#111] border border-white/10 rounded-2xl overflow-hidden shadow-2xl">
          <div className="flex items-center gap-2 px-5 py-3 bg-[#0a0a0a] border-b border-white/8">
            <div className="flex gap-1.5"><div className="w-3 h-3 rounded-full bg-red-500/40"/><div className="w-3 h-3 rounded-full bg-yellow-500/40"/><div className="w-3 h-3 rounded-full bg-green-500/40"/></div>
            <span className="text-white/25 text-xs ml-3">AI Pipeline · Run #247</span>
          ...
          <div className="p-8">
                ...
                <React.Fragment key={i}>
                  <div className="flex flex-col items-center gap-3 shrink-0">
                    ...
                    <div className="text-center">
                      ...
                      <div className={\`text-xs mt-0.5 \${n.status==='done'?'text-emerald-400':n.status==='active'?'text-[#FF3D57]':'text-white/20'}\`}>{n.stat}</div>
                    </div>
                  </div>
                  {i < nodes.length - 1 && (
            ...
            <div className="bg-black/40 border border-white/8 rounded-xl p-5 font-mono text-xs space-y-1.5">
              <div className="text-white/25 mb-3 font-sans text-[11px] uppercase tracking-wider">Live Output</div>
              {logs.map((l, i) => (
                <div key={i} className="flex items-start gap-3">
    ...
    <section className="py-24 bg-[#0F0F0F]">
      <div className="max-w-4xl mx-auto px-6">
        <div className="mb-16">
          ...
          <p className="text-white/30 text-base">PRICING_SUBHEADING</p>
        </div>
        <div className="grid grid-cols-3 gap-0 border border-white/8 rounded-2xl overflow-hidden">
          <div className="border-r border-white/8 p-6">
            <div className="text-white/20 text-xs font-semibold uppercase tracking-wider mb-16">Plan</div>
          </div>
          {plans.map((p, i) => (
            <div key={i} className={\`p-6 \${i===0?'border-r border-white/8':''} \${p.active?'bg-white/3':''}\`}>
              <div className="text-white/25 text-xs font-semibold uppercase tracking-wider mb-4">{p.name}</div>
              <div className="mb-4">
                <span className="text-white font-black text-4xl">{p.price}</span>
                <span className="text-white/25 text-xs ml-2">{p.period}</span>
              </div>
              <button className={\`w-full py-2.5 rounded-lg text-sm font-semibold transition-all \${p.active?'bg-[#5E6AD2] text-white hover:bg-[#7B83E0]':'border border-white/15 text-white/50 hover:border-white/30'}\`}>{p.cta}</button>
            </div>
          ))}
          {features.map((f, i) => (
            <React.Fragment key={i}>
              ...
              <tr>
                ...
                {tiers.map((t, i) => (
                  <th key={i} className={\`p-4 text-center rounded-t-2xl \${t.popular?'bg-[#635BFF]/20 border border-[#635BFF]/30 border-b-0':''}\`}>
                    ...
                    <button className={\`w-full py-2.5 rounded-xl text-sm font-semibold \${t.popular?'bg-gradient-to-r from-[#635BFF] to-[#00D4FF] text-white':'border border-white/20 text-white hover:bg-white/8'}\`}>{t.cta}</button>
                  </th>
                ))}
              </tr>
            </thead>
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 gap-8 items-stretch">
          ...
          <div className="bg-gradient-to-br from-violet-900/40 to-blue-900/30 border border-violet-500/30 rounded-3xl p-10 flex flex-col justify-between">
            ...
            <button className="mt-8 w-full bg-gradient-to-r from-violet-600 to-blue-600 text-white font-bold py-4 rounded-xl hover:opacity-90 transition-opacity">Contact sales →</button>
          </div>
        </div>
      </div>
    </section>
    ...
    <section className="py-24 bg-[#0B0B0B] overflow-hidden">
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          ...
          <p className="text-white/30 text-base max-w-xs">PRICING_SUBHEADING</p>
        </div>
          ...
          {plans.map((p, i) => (
            ...
            >
              <div className="flex items-start justify-between mb-6">
                <div>
                  <div className="text-white/40 text-xs font-semibold uppercase tracking-wider mb-2">{p.name}</div>
                  <div className="flex items-baseline gap-1">
                    <span className="text-white font-black text-5xl">{p.price}</span>
                    {p.price !== '$0' && <span className="text-white/30 text-sm">/mo</span>}
                  </div>
                </div>
              ...
              <p className="text-white/35 text-sm mb-6 leading-relaxed">{p.desc}</p>
              ...
              {i === 2 && (
                <button className="mt-6 w-full bg-gradient-to-r from-[#FF3D57] to-[#FF6B35] text-white font-bold py-3.5 rounded-xl hover:opacity-90 transition-all">{p.cta} →</button>
              )}
            </div>
          ))}
        </div>
        ...
        <div className="grid md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-white/8 border border-white/8 rounded-2xl overflow-hidden">
          {tiers.map((t, i) => (
            <div key={i} className={\`p-8 flex flex-col gap-8 \${t.primary?'bg-white/3':''}\`}>
              <div>
                <div className={\`text-xs font-semibold uppercase tracking-widest mb-4 \${t.primary?'text-white':'text-white/35'}\`}>{t.name}</div>
                <div className="flex items-baseline gap-2 mb-1">
                  <span className="text-white font-black text-4xl">{t.price}</span>
                </div>
                <div className="text-white/25 text-xs">{t.period}</div>
              </div>
              <button className={\`w-full py-3 rounded-lg text-sm font-bold transition-all \${t.primary?'bg-white text-black hover:bg-white/90':'border border-white/15 text-white/50 hover:border-white/30 hover:text-white/70'}\`}>{t.cta}</button>
              <div className="flex flex-col gap-3">
                <div className="text-white/20 text-xs uppercase tracking-wider font-semibold">Included</div>
                {t.features.map((f, fi) => (
                  <div key={fi} className="flex items-center gap-2.5">
                    <div className={\`w-1.5 h-1.5 rounded-full shrink-0 \${t.primary?'bg-white':'bg-white/25'}\`}/>
                    <span className={\`text-sm \${t.primary?'text-white/60':'text-white/30'}\`}>{f}</span>
                  </div>
                ))}
              </div>
            </div>
