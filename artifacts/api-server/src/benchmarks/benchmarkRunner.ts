...
function saveResult(result: BenchmarkResult): void {
  ...
  logger.info('Benchmark result saved', { provider: result.provider, promptId: result.promptId, overall: result.evaluatorScores.overall });
}
...
export async function runVoxAIBenchmark(
  ...
  logger.info('Starting VoxAI benchmark build', { promptId: prompt.id });
  const t0 = Date.now();
  ...
  try {
    ...
    logger.error('VoxAI build failed', { promptId: prompt.id, err: String(err) });
  }
  ...
  // V7.3.4: visual analysis for benchmark
  const visualResult = html ? analyzeVisuals(html, prompt.expectedSections, prompt.id, prompt.id) : null;
  ...
  const result: BenchmarkResult = {
    ...
    evaluatorScores: {
      ...
      visualScore:      visualResult?.visualScore      ?? 0,
      heroVisual:       visualResult?.heroScore        ?? 0,
      ctaVisual:        visualResult?.ctaScore         ?? 0,
      layoutVisual:     visualResult?.layoutScore      ?? 0,
      responsiveVisual: visualResult?.responsiveScore  ?? 0,
    },
...
export async function importCompetitorResult(
  ...
  // V7.3.4: visual analysis for competitor benchmarks
  const visualResult = analyzeVisuals(html, [], `competitor-${provider}-${promptId}`, promptId);
  ...
  const result: BenchmarkResult = {
    ...
    evaluatorScores: {
      ...
      heroVisual:      visualResult.heroScore,
      ctaVisual:       visualResult.ctaScore,
      layoutVisual:    visualResult.layoutScore,
      responsiveVisual: visualResult.responsiveScore,
    },
...
export async function runFullBenchmark(options: {
  ...
  for (const prompt of prompts) {
    if (options.skipExisting) {
      ...
      if (existing) {
        logger.info('Skipping (already captured)', { promptId: prompt.id });
